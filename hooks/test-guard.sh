...
check block 'git push origin +main'
check block 'gh repo delete example/DeepAPI --yes'
...
check block 'gh api -X DELETE /repos/example/DeepAPI'
check block 'gh api repos/example/DeepAPI --method DELETE'
check block 'gh api --method=delete /repos/x/y'
check block 'gh repo edit example/DeepAPI --visibility public'
...
check allow 'rm -rf ~/code/example/tmp/test-dir'
...
check allow 'gh repo view example/DeepAPI'
check allow 'gh repo clone example/DeepAPI'
check allow 'gh api /repos/example/DeepAPI'
...
check allow 'gh repo edit example/DeepAPI --description \"new desc\"'
