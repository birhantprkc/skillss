...
check block 'git push origin +main'
check block 'gh repo delete user/repo --yes'
...
check block 'gh api -X DELETE /repos/user/repo'
check block 'gh api repos/user/repo --method DELETE'
check block 'gh api --method=delete /repos/x/y'
check block 'gh repo edit user/repo --visibility public'
...
check allow 'rm -rf ~/code/project/tmp/bash-guard'
...
check allow 'gh repo view user/repo'
check allow 'gh repo clone user/repo'
check allow 'gh api /repos/user/repo'
...
check allow 'gh repo edit user/repo --description "new desc"'
