...
check block 'git push origin +main'
check block 'gh repo delete example-owner/DeepAPI --yes'
...
check block 'gh api -X DELETE /repos/example-owner/DeepAPI'
check block 'gh api repos/example-owner/DeepAPI --method DELETE'
check block 'gh api --method=delete /repos/x/y'
check block 'gh repo edit example-owner/DeepAPI --visibility public'
...
check allow 'rm -rf ~/old-project/tmp/test-guard'
...
check allow 'gh repo view example-owner/DeepAPI'
check allow 'gh repo clone example-owner/DeepAPI'
check allow 'gh api /repos/example-owner/DeepAPI'
...
check allow 'gh repo edit example-owner/DeepAPI --description "new desc"'
