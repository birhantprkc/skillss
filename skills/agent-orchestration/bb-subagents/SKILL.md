...
If bb reports the worker is blocked on a command, file change, or permission, unblock it yourself. Do not wait for David. Quickly read WHAT it wants to run and WHY (`bb thread interactions list <id> --json`), and if it fits the brief, approve it (`bb thread interactions approve <interactionId> <id>`). Only escalate to David when the action is destructive, touches prod, or falls outside the brief.
