...
When launching a Pi Agent inside a cmux pane and polling for output, use **short `sleep` intervals (2–5s)**. Pi is fast and minimal, and streams tokens extremely quickly on fast models. Do NOT use `sleep 15` unless genuinely needed (a big build/refactor) — most of the time `sleep 2`–`sleep 5` is more than enough.
...
Keep BOTH off — don't re-enable. `sidebar.hideAllDetails: true` hides the entire detail block (status, branch, cwd) if ever needed.
