...
You'll need an OpenRouter API key for this benchmark. Export it in your shell profile (e.g. `~/.zshrc` or `~/.bashrc`) and make sure it's available in a fresh shell as `OPENROUTER_API_KEY`. If it's not set, re-source the shell:
