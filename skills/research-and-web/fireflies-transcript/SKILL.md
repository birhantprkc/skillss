...
The API key lives in a local env file (mode 600, never commit or print it); point `FIREFLIES_ENV_FILE` at its path if it is not already set:

```bash
source "$FIREFLIES_ENV_FILE"   # exports FIREFLIES_API_KEY
...
- 401/invalid key — key was rotated; ask the user for a new one (Fireflies dashboard:
  Settings -> Developer settings), update your local Fireflies env file.
