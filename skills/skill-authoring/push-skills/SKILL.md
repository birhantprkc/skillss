---
name: push-skills
description: Commit and push changes to a skills repo, including global AGENTS.md. Use after editing skills, or when the user says "push the skill", "push skills to github", "update the skills repo", or "publish code/AGENTS.md". Covers private commits and public mirror verification; use publish-skill for skill allowlisting.
---

# Push Skills

Use the configured private repository and follow its established publication process. Only commit or push when the user requests it. Editing this skill is not permission to publish it. For distributed skill edits, follow `distribute-skill-to-all-agents` before pushing.

## Always include global instructions

The user's standing scope for an authorized `/push-skills` run includes saved changes to `<global-instructions-path>`, unless they explicitly exclude that file. Check it on every run, even when the latest message names a skill. Preserve the conversation's original publishing objective through skill edits, renames, and other follow-up requests.

After fetching, compare the saved global instructions with the corresponding file at `origin/main`. A dirty local checkout may simply lag behind an already published commit; do not use local Git status alone to decide whether the saved content is committed. Transfer genuinely new edits while preserving upstream changes.
...
- `<global-instructions-path>` may be a symlink to the saved global instructions. Save the editor buffer and confirm the link before committing. If the link differs, inspect both files before choosing the source; do not replace the link or overwrite either file as part of publishing. The public destination is root `AGENTS.md`.
- The private repository's own `AGENTS.md` contains private repo instructions. It is a different file and stays excluded.
...
Run Git in the private repository, or the temporary worktree described below. Check both staged and unstaged changes:

```bash
git status --short
git diff --cached --stat
git diff HEAD -- <global-instructions-path>
git remote -v
git fetch origin main
```

Replace `<global-instructions-path>` with the exact intended paths for a skill update. Confirm that `origin` is the intended private repo. Review outgoing commits as well as file changes; a push includes every commit ahead of `origin/main`.
...
```bash
git add -- <global-instructions-path>
git commit --only -m "Update global agent instructions" -- <global-instructions-path>
```
...
A private push alone does not prove publication. Find the publication workflow for the pushed commit using the repository's configured CI tools. Wait for that run, not an older successful run. If a newer run supersedes it, confirm that run's source contains the intended change. If the run fails, is cancelled without replacement, or remains stalled, inspect its status and report publication incomplete with the run link. Do not rerun indefinitely.

Read the actual published file and confirm the intended update. For global instructions, check the public root `AGENTS.md`. For skills, check `skills/<category>/<skill-name>/SKILL.md`; use the committed policy and published tree to find the category. Private-only skills need only private push verification.

The mirror may rewrite or omit content. A green workflow and an existing file are not enough: inspect the content. For missing or unexpected output, inspect the workflow's report and documentation. Report exclusions or failures accurately; do not bypass the sanitizer or add manual approval gates. Finish with the private commit, workflow result, and public file link when published.
