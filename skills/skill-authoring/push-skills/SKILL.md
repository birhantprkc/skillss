...
The canonical repo is `~/.agents`, with private remote `private-skills`. Pushes to `main` trigger the sanitized public mirror at `davidondrej/skills`. Only the pipeline pushes to the public repo.
...
The mirror may rewrite or omit content. A green workflow and an existing file are not enough: inspect the content. For missing or unexpected output, inspect that run's mirror report artifact and the mirror tooling docs. Report exclusions or failures accurately; do not bypass the mirror checks or add manual approval gates. Finish with the private commit, workflow result, and public file link when published.
