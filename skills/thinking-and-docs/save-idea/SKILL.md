---
name: save-idea
description: 'Quickly capture an idea from any repo or chat. Everything goes to the ~/ideas-repo repo: video ideas to VIDEO-IDEAS.md; smaller podcast topics, guest ideas, questions, and AI observations to TOPICS.md; marketing and distribution ideas to MARKETING-IDEAS.md; startup ideas to STARTUP-IDEAS.md. Every entry gets a source line referencing the chat and repo it came from. Use when the user says "/save-idea", "save this idea", "video idea", "add a topic", "marketing idea", "startup idea", "write this down for a video/podcast". Differentiator: appends to the user''s idea backlogs — not a reminder, task, or general note tool.'
---

# save-idea

Capture one thing fast, then get out of the way. Four buckets, four files, ONE repo (`~/ideas-repo`):
...
2. **Route it.**
   ...
   - No prefix → judge: business/product idea → startup idea; a way to get an existing product in front of people → marketing idea; full video concept → video idea; smaller thought → topic. Only if genuinely ambiguous, ask the user one short question.
3. **Read the target file** and find the last entry number. Next number = last + 1.
   - `TOPICS.md` and `MARKETING-IDEAS.md` start at 1. `VIDEO-IDEAS.md` continues from an old Google Doc.
...
git -C ~/ideas-repo add VIDEO-IDEAS.md TOPICS.md MARKETING-IDEAS.md STARTUP-IDEAS.md
...
```
   ...
   - Commit message: `Add idea NNNN on <short description>` (or `Add topic NNNN on ...` / `Add marketing idea NN on ...` / `Add startup idea NN on ...`). Multiple entries → list the numbers.
...
- **Committing and pushing the idea files is REQUIRED and pre-authorized by the user** (26-07-2026; startup bucket 01-08-2026). This is a deliberate exception to the global "never push to GitHub by yourself" rule in `~/path/to/AGENTS.md`. Do not "fix" this back — the whole point is that the user never has to commit idea entries by hand.
- The exception covers `VIDEO-IDEAS.md`, `TOPICS.md`, `MARKETING-IDEAS.md`, and `STARTUP-IDEAS.md` only. Any other change in the repo is still the user's to commit.
...
- Indent context lines to match the target file: real tabs in `VIDEO-IDEAS.md`, `TOPICS.md`, and `MARKETING-IDEAS.md`, 4 spaces in `STARTUP-IDEAS.md`.
