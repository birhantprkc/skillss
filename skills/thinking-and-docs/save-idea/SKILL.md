---
name: save-idea
description: 'Quickly capture an idea from any repo or chat. Everything goes to the ~/ideas repo: video ideas to VIDEO-IDEAS.md; smaller podcast topics, guest ideas, questions, and AI observations to TOPICS.md; marketing and distribution ideas to MARKETING-IDEAS.md; startup ideas to STARTUP-IDEAS.md; convictions the user is certain about on AI-era building to CONVICTIONS.md; mini/technical/cool projects to build to mini-projects.md. Every entry gets a source line referencing the chat and repo it came from. Use when the user says "/save-idea", "save this idea", "video idea", "add a topic", "marketing idea", "startup idea", "save this conviction", "mini project", "cool project to build", "write this down for a video/podcast". Differentiator: appends to the user''s idea backlogs — not a reminder, task, or general note tool.'
---

# save-idea

Capture one thing fast, then get out of the way. Seven buckets, seven files, ONE repo (`~/ideas`):

| Bucket | File | What belongs there |
|---|---|---|
| Video idea | `~/ideas/VIDEO-IDEAS.md` | A concept big enough for a full video |
| Topic | `~/ideas/TOPICS.md` | Smaller stuff: podcast topics, guests, questions, AI observations |
| Marketing idea | `~/ideas/MARKETING-IDEAS.md` | Marketing and distribution: how we get products in front of people |
| Startup idea | `~/ideas/STARTUP-IDEAS.md` | A business/product idea — a candidate for the user's next startup |
| Conviction | `~/ideas/CONVICTIONS.md` | A belief the user is CERTAIN about on AI-era building (startups, AI industry, agentic coding) — a conviction, not an idea |
| Mini project | `~/ideas/mini-projects.md` | A small technical/cool project to build — just cool shit, not a startup candidate |
| Article idea | `~/ideas/ARTICLE-IDEAS.md` | A topic for the user's daily public articles (X, blog, YouTube community, email) |
...
2. **Route it.**
   ...
   - Convictions have NO prefix (deliberate, the user 2026-08-18) — route them by judgment only.
...
```
   ...
   - **Startup ideas** — insert after the last numbered idea, keeping the trailing `----` separator and discarded-ideas note as the last lines of the file. Format per `~/ideas/startup/AGENTS.md`: one tight entry, `N. Title — one-line explanation.` Rewrite for clarity and concision but stay loyal to the user's wording and voice. Blank line between entries. Source line indented with 4 spaces (that file uses spaces, not tabs):

```
69. Idea title — one-line explanation in the user's voice.
    - source: ~/some-repo, Cursor chat "Chat title", 2026-08-01
```
...
1. Punchy conviction — one-line why.
    - source: ~/some-repo, Cursor chat "Chat title", 2026-08-18
```
...
10. Idea title exactly as the user said it
    - source: ~/some-repo, Cursor chat "Chat title", 2026-08-24
```
...
```bash
git -C ~/ideas pull --rebase --autostash origin main
git -C ~/ideas add VIDEO-IDEAS.md TOPICS.md MARKETING-IDEAS.md ARTICLE-IDEAS.md STARTUP-IDEAS.md CONVICTIONS.md mini-projects.md
git -C ~/ideas commit -m "Add idea 4839 on graph engineering video"
git -C ~/ideas push origin main
```
...
- All idea capture was centralized into `~/ideas` on 2026-08-14 (the user's decision). The old `~/next-startup` repo is frozen pending archive — NEVER write or push there. Startup support material now lives in `~/ideas/startup/`.
- NEVER write to `~/ideas/startup/review/DISCARDED.md` — only the user moves ideas there.
