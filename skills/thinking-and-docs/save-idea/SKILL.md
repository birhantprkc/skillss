---
name: save-idea
description: 'Quickly capture an idea from any repo or chat. Everything goes to the ~/code/ideas repo: video ideas to VIDEO-IDEAS.md; smaller podcast topics, guest ideas, questions, and AI observations to TOPICS.md; marketing and distribution ideas to MARKETING-IDEAS.md; startup ideas to STARTUP-IDEAS.md; convictions David is certain about on AI-era building to CONVICTIONS.md; mini/technical/cool projects to build to mini-projects.md. Every entry gets a source line referencing the chat and repo it came from. Use when David says "/save-idea", "save this idea", "video idea", "add a topic", "marketing idea", "startup idea", "save this conviction", "mini project", "cool project to build", "write this down for a video/podcast". Differentiator: appends to David''s idea backlogs — not a reminder, task, or general note tool.'
---

# save-idea

Capture one thing fast, then get out of the way. Seven buckets, seven files, ONE repo (`~/code/ideas`):
...
| Startup idea | `~/code/ideas/STARTUP-IDEAS.md` | A business/product idea — a candidate for David's next startup |
| Conviction | `~/code/ideas/CONVICTIONS.md` | A belief David is CERTAIN about on AI-era building (startups, AI industry, agentic coding) — a conviction, not an idea |
| Mini project | `~/code/ideas/mini-projects.md` | A small technical/cool project to build — just cool shit, not a startup candidate |
| Article idea | `~/code/ideas/ARTICLE-IDEAS.md` | A topic for David's daily public articles (X, blog, YouTube community, email) |
...
1. **Get the text.** Everything after `/save-idea` is the entry. Keep David's wording verbatim — never rephrase, shorten, or "improve" it. (One exception: startup ideas get tightened per their repo's rules — see step 4.)
2. **Route it.**
   ...
   - No prefix → judge: a belief stated as a certainty about AI-era building (startups, AI industry, agentic coding) → conviction; business/product idea → startup idea; a small technical/cool project to build (not a company) → mini project; a way to get an existing product in front of people → marketing idea; full video concept → video idea; smaller thought or observation → topic. Only if genuinely ambiguous, ask David one short question.
...
```
NNNN. Idea title exactly as David said it
	source: ~/code/some-repo, Cursor chat "Chat title", 2026-07-15
	any extra links or notes David gave
```

   - **Startup ideas** — insert after the last numbered idea, keeping the trailing `----` separator and discarded-ideas note as the last lines of the file. Format per `~/code/ideas/startup/AGENTS.md`: one tight entry, `N. Title — one-line explanation.` Rewrite for clarity and concision but stay loyal to David's wording and voice. Blank line between entries. Source line indented with 4 spaces (that file uses spaces, not tabs):

```
69. Idea title — one-line explanation in David's voice.
    - source: ~/code/some-repo, Cursor chat "Chat title", 2026-08-01
```

   - **Convictions** — append at the bottom of `CONVICTIONS.md`, same tight style as startup ideas: `N. Punchy conviction — one-line why.` Tighten for punch but stay loyal to David's wording and voice. Blank line between entries, source line indented with 4 spaces:
...
```
10. Idea title exactly as David said it
...
git -C ~/code/ideas add VIDEO-IDEAS.md TOPICS.md MARKETING-IDEAS.md ARTICLE-IDEAS.md STARTUP-IDEAS.md CONVICTIONS.md mini-projects.md
...
```
   ...
   - If the pull or push fails, report the exact error to David and stop. Never force-push.
7. **Confirm back to David**: the exact entry text, its number, which file it went to, and that it was pushed.

## Rules

- Append only. Never edit, reorder, or renumber existing entries.
- Multiple ideas in one invocation → one numbered entry each.
- **Committing and pushing the idea files is REQUIRED and pre-authorized by David** (26-07-2026; startup bucket 01-08-2026; convictions bucket 18-08-2026; mini-projects bucket 24-08-2026). This is a deliberate exception to the global "never push to GitHub by yourself" rule in `~/code/AGENTS.md`. Do not "fix" this back — the whole point is that David never has to commit idea entries by hand.
- The exception covers `VIDEO-IDEAS.md`, `TOPICS.md`, `MARKETING-IDEAS.md`, `ARTICLE-IDEAS.md` (added 26-08-2026), `STARTUP-IDEAS.md`, `CONVICTIONS.md`, and `mini-projects.md` only. Any other change in the repo is still David's to commit.
...
- NEVER write to `~/code/ideas/startup/review/DISCARDED.md` — only David moves ideas there.
